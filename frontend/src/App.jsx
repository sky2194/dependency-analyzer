  ...
  return (
    <ScanContext.Provider value={{ scanning, setScanning, scanProject, setScanProject }}>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
        <nav style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 20px', display: 'flex', alignItems: 'center', gap: 4, height: 56, position: 'sticky', top: 0, zIndex: 100 }}>
          ...
          {[
            ...
            <NavLink key={to} to={to} end={end} style={({ isActive }) => ({
              padding: '7px 14px', fontSize: 13.5, fontWeight: 600, borderRadius: 6,
          ...
          {scanning && <div style={{ marginLeft: 12 }}><ScanStatusBar scanning={scanning} scanProject={scanProject} /></div>}
          <button onClick={toggleTheme} title={`Switch to ${theme === 'dark' ? 'light' : 'dark'} mode`}
            style={{ marginLeft: 'auto', background: 'none', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 11px', cursor: 'pointer', fontSize: 14, color: 'var(--text-2)', transition: 'all 0.15s', flexShrink: 0 }}>
            {theme === 'dark' ? '☀️' : '🌙'}
          </button>
        </nav>
