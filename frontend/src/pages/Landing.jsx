...
const check = (
  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#3ecf8e" strokeWidth="3">
...
function CheckRow({ title, text }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
      ...
      <div>
        <div style={{ fontSize: 14, fontWeight: 600, color: '#eef0f3' }}>{title}</div>
        <div style={{ fontSize: 13, color: '#8b95a8', marginTop: 2 }}>{text}</div>
      </div>
    </div>
  )
}
    ...
    <div className="landing-page">
      ...
      <section className="lp-hero">
        ...
        <svg className="lp-hero-graph" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
          <defs><radialGradient id="ng" cx="50%" cy="40%"><stop offset="0%" stopColor="#4f8ef7" stopOpacity="0.15" /><stop offset="100%" stopColor="#4f8ef7" stopOpacity="0" /></radialGradient></defs>
          <rect fill="url(#ng)" width="1440" height="900" />
          <g stroke="rgba(255,255,255,0.06)" strokeWidth="1" fill="none" strokeDasharray="6 4">
          ...
          <g stroke="rgba(255,59,92,0.2)" strokeWidth="1.5" fill="none"><line x1="360" y1="380" x2="200" y2="560" /><line x1="900" y1="420" x2="840" y2="600" /></g>
          <circle cx="720" cy="200" r="16" stroke="#4f8ef7" strokeWidth="1.5" fill="rgba(79,142,247,0.1)" style={{ animation: 'float 4s ease-in-out infinite' }} />
          <circle cx="560" cy="420" r="10" stroke="rgba(255,255,255,0.15)" strokeWidth="1" fill="rgba(255,255,255,0.03)" style={{ animation: 'float 5s ease-in-out infinite 0.5s' }} />
          <circle cx="720" cy="460" r="10" stroke="rgba(255,255,255,0.12)" strokeWidth="1" fill="rgba(255,255,255,0.02)" style={{ animation: 'float 3.5s ease-in-out infinite 1s' }} />
          {[[360,380,14,'#ff3b5c','rgba(255,59,92,0.12)'],[1060,380,12,'#ff3b5c','rgba(255,59,92,0.08)'],[900,420,12,'#ff8c42','rgba(255,140,66,0.1)'],[200,560,10,'#ff3b5c','rgba(255,59,92,0.1)'],[840,600,10,'#ff8c42','rgba(255,140,66,0.08)']].map(([cx,cy,r,stroke,fill], i) => (
        ...
        <div className="lp-hero-content">
          ...
          <div className="lp-hero-actions">
            ...
            <button onClick={sample} className="lp-btn-hero-ghost">View sample report ›</button>
          </div>
        ...
        <div className="lp-showcase-inner">
          <div className="lp-showcase-header">
            ...
            <div className="lp-showcase-screen">
              <div className="lp-screen-bar"><span style={{ background: '#ff5f57' }} /><span style={{ background: '#ffbd2e' }} /><span style={{ background: '#28ca41' }} /><div>depanalyzer · security risk overview</div></div>
              <div className="lp-screen-body">
                <div className="lp-mini-stats">
                  {['72|Risk Score|#ff3b5c','10|Packages|#eef0f3','2|Critical|#ff3b5c','2|High|#ff8c42'].map(x => {
                    const [v, l, c] = x.split('|')
          ...
          <div className="lp-terminal reveal">
            <div className="lp-terminal-bar"><span style={{ background: '#ff5f57' }} /><span style={{ background: '#ffbd2e' }} /><span style={{ background: '#28ca41' }} /><div>depanalyzer scan</div></div>
      ...
      <div className="lp-cta-band reveal">
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}><button onClick={() => navigate('/scan')} className="lp-btn-hero">→ Scan Now for Free</button><button onClick={sample} className="lp-btn-hero-ghost">View Sample Report</button></div>
      </div>
...
.lp-nav-logo-icon{width:28px;height:28px;background:linear-gradient(135deg,var(--accent),var(--accent2));border-radius:7px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
.lp-nav-links{display:flex;gap:2px;margin-left:32px;flex:1}.lp-nav-link{padding:6px 14px;border-radius:7px;font-size:13.5px;color:var(--text-2);text-decoration:none;transition:all .15s;background:none;border:0;cursor:pointer}.lp-nav-link:hover{color:var(--text);background:rgba(255,255,255,.05)}.lp-nav-cta{display:flex;gap:10px;align-items:center}
...
.lp-btn-ghost{padding:8px 18px;font-size:13.5px;color:var(--text-2);background:transparent;border:1px solid var(--border-mid)}.lp-btn-ghost:hover{color:var(--text);border-color:var(--border-strong)}
.lp-btn-primary{padding:8px 20px;font-size:13.5px;font-weight:600;color:white;background:var(--accent);border:none}.lp-btn-primary:hover,.lp-btn-hero:hover{background:#6ba0f9;transform:translateY(-1px);box-shadow:0 8px 20px rgba(79,142,247,.3)}
.lp-hero{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:120px 48px 80px;position:relative;overflow:hidden;text-align:center}.lp-hero-bg{position:absolute;inset:0;z-index:0;background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(79,142,247,.08) 0%,transparent 70%),radial-gradient(ellipse 50% 40% at 80% 80%,rgba(124,58,237,.06) 0%,transparent 60%)}.lp-hero-graph{position:absolute;inset:0;z-index:0;opacity:.35}.lp-hero-content{position:relative;z-index:1;max-width:820px}.lp-hero-badge{display:inline-flex;align-items:center;gap:8px;padding:5px 14px;border-radius:999px;background:rgba(79,142,247,.08);border:1px solid rgba(79,142,247,.2);font-size:12.5px;font-weight:500;color:var(--accent);margin-bottom:28px}.lp-hero-badge-dot{width:6px;height:6px;border-radius:50%;background:var(--accent);animation:blink 2s infinite}.lp-hero-title{font-family:var(--font-d);font-size:clamp(42px,5.5vw,72px);font-weight:800;letter-spacing:-2.5px;line-height:1.05;color:var(--text);margin-bottom:24px}.lp-hero-title span{background:linear-gradient(135deg,var(--accent) 0%,#a78bfa 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}.lp-hero-sub{font-size:17px;color:var(--text-2);max-width:560px;margin:0 auto 40px;line-height:1.75}.lp-hero-sub strong{color:var(--text);font-weight:500}.lp-hero-actions{display:flex;gap:12px;justify-content:center;align-items:center;flex-wrap:wrap}.lp-btn-hero{padding:14px 32px;border-radius:10px;font-size:15px;font-weight:600;color:white;background:var(--accent);border:none}.lp-btn-hero-ghost{padding:14px 28px;border-radius:10px;font-size:15px;color:var(--text-2);background:transparent;border:1px solid var(--border-mid)}.lp-btn-hero-ghost:hover{color:var(--text);border-color:var(--border-strong);background:rgba(255,255,255,.04)}.lp-hero-stats{display:flex;gap:40px;justify-content:center;flex-wrap:wrap;margin-top:72px;padding-top:48px;border-top:1px solid var(--border)}.lp-hero-stat-val{font-family:var(--font-d);font-size:32px;font-weight:800;letter-spacing:-1px;color:var(--text)}.lp-hero-stat-label{font-size:13px;color:var(--text-2);margin-top:4px}
...
.lp-problem-grid,.lp-features-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--border);border-radius:16px;overflow:hidden;margin-top:56px}.lp-problem-card,.lp-feat-card{background:var(--bg-card);padding:36px 32px}.lp-problem-num{font-family:var(--font-d);font-size:52px;font-weight:800;letter-spacing:-2px;color:rgba(255,255,255,.06);line-height:1;margin-bottom:16px}.lp-problem-title,.lp-feat-title{font-family:var(--font-d);font-size:18px;font-weight:700;color:var(--text);margin-bottom:10px}.lp-problem-text,.lp-feat-text{font-size:14px;color:var(--text-2);line-height:1.75}.lp-features-grid{border-radius:20px}.lp-feat-card:hover{background:var(--bg-elevated)}.lp-feat-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;margin-bottom:20px;font-size:20px;background:rgba(79,142,247,.1);border:1px solid rgba(79,142,247,.2)}
.lp-showcase{background:linear-gradient(180deg,var(--bg) 0%,#0b0f17 50%,var(--bg) 100%);padding:96px 48px}.lp-showcase-inner{max-width:1200px;margin:0 auto}.lp-showcase-header,.lp-two-col{display:grid;grid-template-columns:1fr 1fr;gap:64px;align-items:center;margin-bottom:64px}.lp-check{width:20px;height:20px;border-radius:5px;background:rgba(62,207,142,.12);border:1px solid rgba(62,207,142,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px}.lp-showcase-screen{background:var(--bg-card);border:1px solid var(--border);border-radius:16px;overflow:hidden;box-shadow:0 40px 80px rgba(0,0,0,.6),0 0 0 1px var(--border);position:relative}.lp-showcase-screen:before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--accent),transparent)}.lp-screen-bar,.lp-terminal-bar{height:40px;background:var(--bg-elevated);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 16px;gap:8px}.lp-screen-bar span,.lp-terminal-bar span{width:10px;height:10px;border-radius:50%}.lp-screen-bar div,.lp-terminal-bar div{flex:1;text-align:center;font-family:var(--font-m);font-size:11px;color:#4a5568}.lp-screen-body{padding:20px}.lp-mini-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px}.lp-mini-stats div{background:var(--bg-elevated);border:1px solid var(--border);border-radius:8px;padding:10px;text-align:center}.lp-mini-stats strong{font-family:var(--font-d);font-size:22px;font-weight:800;display:block}.lp-mini-stats span{font-size:10px;color:#8b95a8}.lp-vuln-row{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;padding:8px 10px;border-radius:6px;margin-bottom:6px;background:rgba(255,255,255,.015)}.lp-vuln-row.critical{background:rgba(255,59,92,.04);border-left:2px solid #ff3b5c}.lp-vuln-row.high{background:rgba(255,140,66,.04)}.lp-vuln-row b{display:block;font-family:var(--font-m);font-size:12px;color:#a8c4e0}.lp-vuln-row small{font-size:10px;color:#4a5568}.lp-vuln-row em{padding:2px 7px;border-radius:4px;font-size:10px;font-weight:600;text-transform:uppercase;font-style:normal;color:#ff8c42;border:1px solid rgba(255,140,66,.3);background:rgba(255,140,66,.15)}.lp-vuln-row.critical em{color:#ff3b5c;border-color:rgba(255,59,92,.3);background:rgba(255,59,92,.15)}.lp-vuln-row.medium em{color:#f5c842;border-color:rgba(245,200,66,.3);background:rgba(245,200,66,.15)}.lp-vuln-row span{font-family:var(--font-m);font-size:10px;color:#4a5568}
.lp-terminal{background:#0a0d12;border:1px solid var(--border-mid);border-radius:12px;overflow:hidden;font-family:var(--font-m);box-shadow:0 20px 60px rgba(0,0,0,.5)}.lp-terminal-body{padding:18px 20px;font-size:12.5px;line-height:1.9;color:#eef0f3}.lp-terminal-body i{color:#505a6b;font-style:normal}.lp-terminal-body strong{color:#3ecf8e}.lp-terminal-body b{color:#4f8ef7}.lp-terminal-body mark{background:transparent;color:#ff3b5c}.lp-terminal-body u{color:#ff8c42;text-decoration:none}
...
.lp-integrations-bg{background:var(--bg-card);border-top:1px solid var(--border);border-bottom:1px solid var(--border);padding:80px 48px}.lp-integrations-inner{max-width:1200px;margin:0 auto}.lp-integration-logos{display:flex;gap:12px;flex-wrap:wrap;margin-top:40px}.lp-integration-badge{display:flex;align-items:center;gap:10px;padding:12px 20px;border-radius:10px;background:var(--bg-elevated);border:1px solid var(--border-mid);font-size:13.5px;font-weight:500;color:var(--text-2);transition:all .2s}.lp-integration-badge:hover{border-color:var(--border-strong);color:var(--text);background:rgba(255,255,255,.05)}.lp-integration-badge span{width:10px;height:10px;border-radius:50%;background:#4f8ef7}.lp-code-card{margin-top:24px;background:#080b10;border:1px solid rgba(255,255,255,.06);border-radius:9px;padding:14px 16px;font-family:var(--font-m);font-size:12px;line-height:1.9;color:#8b95a8}.lp-code-card div{color:#4a5568}.lp-code-card b{color:#7c3aed}.lp-code-card span{color:#4f8ef7}.lp-code-card em{color:#3ecf8e;font-style:normal}
.lp-testimonials,.lp-pricing-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px;margin-top:56px}.lp-testi-card,.lp-price-card{background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:28px;display:flex;flex-direction:column;gap:20px;transition:border-color .2s}.lp-testi-card:hover,.lp-price-card:hover{border-color:var(--border-mid)}.lp-testi-quote{font-size:14.5px;color:var(--text-2);line-height:1.8;font-style:italic}.lp-testi-author{display:flex;align-items:center;gap:12px}.lp-testi-avatar{width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-family:var(--font-d);font-size:14px;font-weight:700;flex-shrink:0;background:rgba(79,142,247,.15);color:#4f8ef7}.lp-testi-name{font-size:13.5px;font-weight:600;color:var(--text)}.lp-testi-role{font-size:12px;color:var(--text-3)}
.lp-price-card{border-radius:16px;padding:32px;position:relative;overflow:hidden}.lp-price-card.featured{border-color:rgba(79,142,247,.3)}.lp-price-card.featured:before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--accent),transparent)}.lp-price-badge{position:absolute;top:16px;right:16px;padding:3px 10px;border-radius:5px;background:rgba(79,142,247,.15);border:1px solid rgba(79,142,247,.3);font-size:11px;font-weight:600;color:var(--accent)}.lp-price-tier{font-size:12px;font-weight:600;letter-spacing:.6px;text-transform:uppercase;color:var(--text-2)}.lp-price-val{font-family:var(--font-d);font-size:40px;font-weight:800;letter-spacing:-1.5px;color:var(--text);line-height:1;margin-top:8px}.lp-price-val span{font-size:16px;font-weight:400;color:var(--text-2)}.lp-price-desc{font-size:13.5px;color:var(--text-2);line-height:1.6;margin-top:10px}.lp-price-features{display:flex;flex-direction:column;gap:10px;flex:1}.lp-price-feat{display:flex;align-items:flex-start;gap:9px;font-size:13.5px;color:var(--text-2)}.lp-price-feat div{width:16px;height:16px;border-radius:4px;background:rgba(62,207,142,.12);border:1px solid rgba(62,207,142,.25);display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px}.lp-price-btn{width:100%;padding:11px;border-radius:9px;font-size:14px;font-weight:600;cursor:pointer;font-family:var(--font-b);transition:all .2s;border:1px solid var(--border-mid);background:transparent;color:var(--text-2)}.lp-price-btn:hover{background:rgba(255,255,255,.05);color:var(--text)}.lp-price-btn.primary{background:var(--accent);border-color:transparent;color:white}.lp-price-btn.primary:hover{background:#6ba0f9}
.lp-cta-band{margin:0 48px 96px;border-radius:20px;background:linear-gradient(135deg,rgba(79,142,247,.12) 0%,rgba(124,58,237,.10) 100%);border:1px solid rgba(79,142,247,.2);padding:72px 64px;display:flex;align-items:center;justify-content:space-between;gap:40px;position:relative;overflow:hidden}.lp-cta-band:before{content:'';position:absolute;top:-60px;right:-60px;width:300px;height:300px;border-radius:50%;background:radial-gradient(circle,rgba(79,142,247,.1) 0%,transparent 70%);pointer-events:none}.lp-cta-title{font-family:var(--font-d);font-size:36px;font-weight:800;letter-spacing:-1px;line-height:1.2;color:var(--text)}.lp-cta-sub{font-size:15px;color:var(--text-2);margin-top:10px}
